...
static inline void kdv_scatter_bound_states_U_ES4(COMPLEX const a1,
                                                  COMPLEX const a2,
                                                  COMPLEX const a3,
...
INT kdv_scatter_bound_states(const UINT D, COMPLEX const * const q,
        ...
        kdv_discretization_t const discretization, UINT const skip_b_flag)
{
    ...
    switch (discretization) {
        ...
        case kdv_discretization_CF6_4_VANILLA: // commutator-free sixth-order
            N++;                               // The previous six discretizations require N=3
            // fall through
    ...
    for (UINT neig=0; neig<K; neig++) { // iterate over bound states
        ...
        ret_code = kdv_discretization_change_of_basis_matrix_from_S(&Tmx[0][0],l_curr,derivative_flag,eps_t,discretization);
        CHECK_RETCODE(ret_code, leave_fun);
        ...
        switch (discretization) {
            ...
            case kdv_discretization_ES4_VANILLA:
                for (UINT n = 0, n_given=0; n<D; n+=3, n_given++) {
                    ...
                    kdv_scatter_bound_states_U_ES4(a1,a2,a3,1,*U,&w,&s,&c);
            ...
            case kdv_discretization_TES4_VANILLA:
                for (UINT n=0, n_given=0; n<D; n+=3, n_given++) {
                    ...
                    kdv_scatter_bound_states_U_ES4(tmp1[n],tmp1[n+1],0.0,1,*U,&w,&s,&c);
                    memcpy(&U[2][2],&U[0][0],6 * sizeof(COMPLEX)); // lower right block
                    misc_matrix_mult(4,4,1,*U,&PHI[4*n_given],&PHI[4*(n_given+1)]);
                    ...
                    kdv_scatter_bound_states_U_ES4(a1,a2,a3,1,*UN,&w,&s,&c);
                    ...
                    kdv_scatter_bound_states_U_ES4(tmp2[n],tmp2[n+1],0.0,1,*U,&w,&s,&c);
                    memcpy(&U[2][2],&U[0][0],6 * sizeof(COMPLEX)); // lower right block
            ...
            switch (discretization) {
                ...
                case kdv_discretization_ES4_VANILLA:
                    for (UINT n_given=D_given, n=D-3; n_given-->0; n-=3) {
                        ...
                        COMPLEX a2 = -tmp1[n+1] + eps_t_3*l_curr*(q[n+1]+r[n+1])/12.0;
                        COMPLEX a3 =  eps_t*I*l_curr -tmp1[n+2];
                        kdv_scatter_bound_states_U_ES4(a1,a2,a3,0,*U,&w,&s,&c);
                ...
                case kdv_discretization_TES4_VANILLA:
                    for (UINT n_given=D_given, n=D-3; n_given-->0; n-=3) {
                        ...
                        kdv_scatter_bound_states_U_ES4(tmp3[n],tmp3[n+1],0.0,0,*U,&w,&s,&c);
                        misc_matrix_mult(2,2,1,*U,&PSI[4*(n_given+1)],&PSI[4*n_given]);

                        kdv_scatter_bound_states_U_ES4(-0.5*eps_t*(q[n]+r[n]), -0.5*I*eps_t*(q[n]-r[n]),eps_t*l_curr*I,0,*U,&w,&s,&c);
                        misc_matrix_mult(2,2,1,*U,&PSI[4*n_given],psi_temp);

                        kdv_scatter_bound_states_U_ES4(tmp4[n],tmp4[n+1],0.0,0,*U,&w,&s,&c);
        ...
        derivative_flag = 1;
        ret_code = kdv_discretization_change_of_basis_matrix_to_S(&Tmx[0][0],l_curr,derivative_flag,eps_t,discretization);
        CHECK_RETCODE(ret_code, leave_fun);
