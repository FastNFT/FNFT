        ...
        kdv_discretization_t const discretization, UINT const skip_b_flag)
{
    ...
    COMPLEX phiS1, phiS2, phiS1_D, phiS2_D;
    ...
    for (neig = 0; neig < K; neig++) { // iterate over bound states
        ...
        PHI2[0] = phiS1 + phiS2;
        PHI1_D = 2.0 * I * phiS1 + 2.0 * I * l_curr * phiS1_D;
        PHI2_D = phiS1_D + phiS2_D;
...
//                         a3 = 0;
        ...
        a_vals[neig] = (-0.5 * I / l_curr) * PHI1[D_given];
        a_vals[neig] *= CEXP(I*l_curr*(T[1]+eps_t*boundary_coeff));
        aprime_vals[neig] = 1.0/l_curr - I * (T[1]+eps_t*boundary_coeff);
        aprime_vals[neig] *= PHI1[D_given];
        aprime_vals[neig] -= PHI1_D;
        aprime_vals[neig] *= scl_factor;
        aprime_vals[neig] *= 0.5 * I / l_curr;
        aprime_vals[neig] *= CEXP(I*l_curr*(T[1]+eps_t*boundary_coeff));
