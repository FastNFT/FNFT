...
{
	// This function multiplies the exponential matrix from FTES4 with E1 and E2, resulting in the TM for this method.
	// The polynomial coefs pij[n] from TM get replaced by the values after E1*TM*E2
	...
	INT* const W_ptr, manakov_discretization_t const discretization)
{
	...
	deg_ptr: pointer to variable containing the degree of the discretization
	W_ptr: normalization flag. polynomial coefficients are normalized if W_ptr is non-zero
	discretization: type of discretization used
	kappa: focussing or defocussing

	r1,2 is not passed here, as for the manakov equation this is always equal to -kappa *conj(q1,2)
	*/

	INT ret_code;
	COMPLEX* p, * p11, * p12, * p13, * p21, * p22, * p23, * p31, * p32, * p33;
	UINT i, j, len, D_eff, upsampling_factor;
	COMPLEX e_Bstorage[36], E_storage[18]; // e_Bstorage length: 9x the max. amount of e_B matrices we need for a discretization
	...
	COMPLEX* E1, * E2;               // matrices used for FTES4 method
	COMPLEX q1n[3], q2n[3];
	...
	// Set up individual scattering matrices
	*deg_ptr = manakov_discretization_degree(discretization);
	...
	case manakov_discretization_2SPLIT3A:
		...
		e_B = &e_Bstorage[9];   // exp(3/3 Bh) = exp(Bh)
...
*/
	...
	case manakov_discretization_FTES4_4A:
		...
		for (i = D_eff; i-- > 0;) {
			...
			manakov_fscatter_get_E(E1, (q1n[2] - q1n[0]) * eps_t / 24 + (q1n[2] - 2 * q1n[1] + q1n[0]) * eps_t / 48,
			...
			manakov_fscatter_get_E(E2, -(q1n[2] - q1n[0]) * eps_t / 24 + (q1n[2] - 2 * q1n[1] + q1n[0]) * eps_t / 48,
				-(q2n[2] - q2n[0]) * eps_t / 24 + (q2n[2] - 2 * q2n[1] + q2n[0]) * eps_t / 48,
