...
ext1 = "sech_2sept_v1";       % what follows after discr_method_ for the first dataset
ext2 = "sech_2sept_v1";       % what follows after discr_method_ for the second dataset
...
% load exact solution
load(strcat("exact_sol_",ext1,'.mat'))
exact_sol = Test_results;

for method_index = 1:length(methods)
    load(strcat(methods(method_index),'_',ext1,'.mat'))
...
%% Plot second dataset lines
for ind = 1:length(methods) % TODO: change colours here
    if strcmp(methods(ind),'discr_2split3A')
        names(ind) = "2split3A";
        line_colour(ind,:) = [1 0 0];
        line_marker(ind) = 'p';
...
% load exact solution
load(strcat("exact_sol_",ext2,'.mat'))
exact_sol = Test_results;

for method_index = 1:length(methods)
    load(strcat(methods(method_index),'_',ext2,'.mat'))
    D= Test_results.params.D_values;
    RMSE = zeros(1,length(D));
    runtime = zeros(1,length(D));
    for i=1:length(D)
