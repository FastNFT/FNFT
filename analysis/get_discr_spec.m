...
% Computes ONLY the bound states
% Runs tests with M=D. Computes exact result in separate file
...
signal = 'two_soliton';    % potential function: sech or rect or soliton
...
if strcmp(discretization,'exact_sol')
    tStart = tic;
    if strcmp(signal,'single_soliton')
        bound_states = xi + 1i*eta;
    elseif strcmp(signal,'two_soliton')
        bound_states = xi + 1i*eta;
    end
...
for ind = 1:3
    tStart = tic;
    [~, bound_states] = mex_fnft_manakovv(complex(q1), complex(q2), T_f, XI, kappa, 'M', M, 'skip_contspec', discretization);
...
save(strcat(discretization,'_',signal,'_1sept_v1'),'Test_results')
