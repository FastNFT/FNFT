                        ...
                        const REAL eps_t, const COMPLEX z, COMPLEX U[4])
{
    ...
    COMPLEX e_plus[4], e_minus[4], e_quarter[4], e_half[4];
    ...
    zero_freq_exponential(e_quarter, eps_t/4.0, q[i], r[i]);
    zero_freq_exponential(e_half, eps_t/2.0, q[i], r[i]);

    matrix_mult(e_quarter, Z, tmp1);
    matrix_mult(tmp1, e_half, tmp2);
    matrix_mult(tmp2, Z, tmp1);
    matrix_mult(tmp1, e_quarter, term1);

    matrix_mult(e_half, Z2, tmp1);
    matrix_mult(tmp1, e_half, term2);
...
static INT run_case(const INT kappa, const UINT normalize)
{
    ...
    numel = akns_fscatter_numel(D, akns_discretization_FTES4_4B);
    if (numel == 0)
        return E_INVALID_ARGUMENT(akns_discretization_FTES4_4B);
...
static INT run_nsev_convergence(void)
{
    ...
    REAL shifted_error_bounds[6] = {
        ...
        1.1e-3, // b
        1.4e-3, // bound states
        2e-14,  // norming constants
        2.2e-3  // residues
    };

    opts = fnft_nsev_default_opts();
    opts.discretization = nse_discretization_FTES4_4B;
...
INT main()
{
    ...
    if (akns_discretization_degree(akns_discretization_FTES4_4B) != 2 ||
        akns_discretization_method_order(akns_discretization_FTES4_4B) != 4)
        return EXIT_FAILURE;
