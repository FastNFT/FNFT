...
INT main()
{
    ...
    for (UINT n=0; n<2; n++){
        ...
        for (UINT i=0; i<4; i++)
            eb_RE[i] /= 8.0;
        for (UINT i=4; i<6; i++)
            eb_RE[i] /= 4.0; // Only 2nd order error decay for norming constants and residues
        ret_code = kdvv_testcases_test_fnft(tc, D, eb_RE, &opts);
        CHECK_RETCODE(ret_code, leave_fun);
    }
