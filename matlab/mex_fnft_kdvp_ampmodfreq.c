...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    ampmodfreq = mxMalloc(3*K * sizeof(mxREAL));
    if (ampmodfreq == NULL) {
    ...
    if (ret_code != FNFT_SUCCESS) {
        snprintf(msg, sizeof msg, "fnft_kdvp_ampmodfreq failed (error code %i).",
                ret_code);
        goto on_error;
    }
    ...
    plhs[0] = mxCreateDoubleMatrix(1, 3*K, mxREAL);
    if (plhs[0] == NULL) {
        snprintf(msg, sizeof msg, "Out of memory.");
        goto on_error;
    }
