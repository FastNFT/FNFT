...
tic;
spines2 = mex_fnft_nsep(q, T, kappa, 'phase_shift', phase_shift, 'points_per_spine', 100, ...
    'loc_newton', ig, complex([0]), 'filt_manual', [-4 4 0 4], ...
    'loc_max_evals', 100);
toc
...
% 
figure;
plot(real(ig), imag(ig), '.', ...
    real(main_spec), imag(main_spec), 'o', ...
    real(main_spec2), imag(main_spec2), 'x');
title('Main Spectrum');
xlabel('Re(\lambda_k)');
ylabel('Im(\lambda_k)');
xlim([-4 4]);
...
figure;
plot(real(ig), imag(ig), '.', ...
    real(aux_spec), imag(aux_spec), 'o', ...
    real(aux_spec2), imag(aux_spec2), 'x');
...
figure;
plot(real(spines), imag(spines), 'xr', ...
     real(spines2), imag(spines2), '.m');
title('Spines');
xlabel('Re(\lambda)');
ylabel('Im(\lambda)');
xlim([-4 4]);
